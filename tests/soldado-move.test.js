...
import { createMask, setAt, solidAt, normalAt, AR, TERRA } from '../js/frente/mask.js';
import { createSoldado, andar, pular, empurrar, atualizar, estaParada, apoiada, colide, VELOCIDADE } from '../js/frente/soldado.js';
import { GRAVIDADE, ARRASTO } from '../js/frente/ballistics.js';

const PPM = 20;
const AMBIENTE = { gravidade: GRAVIDADE, arrasto: ARRASTO, vento: 0 };

/**
 * Terreno de teste: só o que o soldado usa (`solidoEm` e `normalEm`), sobre
...
/** Assenta o soldado no chão antes do teste começar. */
function assentar(w, terreno, segundos = 2) {
  for (let i = 0; i < segundos * 120; i += 1) atualizar(w, terreno, 1 / 120, AMBIENTE);
}

test('o soldado cai e assenta em cima do chão', () => {
  const t = terrenoPlano();
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 10, y: 12 });
...
test('andar move o soldado na velocidade certa, em qualquer passo de tempo', () => {
  ...
  for (const passos of [30, 60, 120, 240]) {
    const t = terrenoPlano();
    const w = createSoldado({ nome: 'Tico', equipe: 0, x: 10, y: 5.2 });
    assentar(w, t);
    const inicio = w.x;
...
test('o soldado sobe uma rampa andando, sem pular', () => {
  const t = terrenoComRampa();
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 12, y: 6 });
  assentar(w, t);
...
test('o soldado desce a rampa colada no chão, sem despencar', () => {
  const t = terrenoComRampa();
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 25, y: 10 });
  assentar(w, t);
...
test('uma parede alta bloqueia o soldado em vez de deixá-lo atravessar', () => {
  ...
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 10, y: 6 });
  assentar(w, t);
  ...
  assert.ok(w.x < 15, `o soldado atravessou a parede (x = ${w.x.toFixed(2)})`);
  assert.ok(!colide(t, w.x, w.y), 'e não pode ficar preso dentro dela (a parede)');
});

test('o soldado não gruda numa quina: ou sobe o degrau, ou para limpa', () => {
  const t = terrenoDeTeste(600, 300, (set) => {
    for (let y = 200; y < 300; y += 1) for (let x = 0; x < 600; x += 1) set(x, y);
    // Degrau de 5 px — dentro do que ele sobe andando.
...
test('o soldado cai quando o chão some debaixo dele', () => {
  const t = terrenoDeTeste(600, 300, (set) => {
    for (let y = 200; y < 300; y += 1) for (let x = 0; x < 600; x += 1) set(x, y);
  });
  ...
  assert.equal(w.estado, 'voando', 'sem chão, o soldado tem de despencar');
});

test('cair de muito alto machuca; de pouco, não', () => {
  const t = terrenoPlano();

  const curta = createSoldado({ nome: 'A', equipe: 0, x: 10, y: 5.6 });
  let danoCurto = 0;
  ...
  const longa = createSoldado({ nome: 'B', equipe: 0, x: 20, y: 26 });
  let danoLongo = 0;
...
test('o empurrão da explosão joga o soldado no ar', () => {
  const t = terrenoPlano();
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 10, y: 5.2 });
  assentar(w, t);
...
test('soldado morto não se mexe mais', () => {
  const t = terrenoPlano();
  const w = createSoldado({ nome: 'Tico', equipe: 0, x: 10, y: 12 });
  w.vivo = false;
  ...
  assert.equal(estaParada(w), true, 'um soldado morto nunca segura o turno');
});
