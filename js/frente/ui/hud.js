...
import { ALTURA as ALTURA_SOLDADO } from '../soldado.js';
...
/**
 * Altura reservada embaixo, na tela estreita, para a barra de controles de
 * batalha (`#battle-controls`, em `linha-de-frente/css/style.css`) — andar,
 * mirar, trocar de arma e o botão FOGO. Sem essa reserva, os blocos de
 * vento/arma e o rodapé de dicas desenhados aqui ficavam por baixo dos
 * botões, ilegíveis.
 */
const RESERVA_CONTROLES = 190;
...
function layout(camera) {
  ...
  const base = camera.height - RESERVA_CONTROLES - 38 - margem;
  return {
    estreito,
...
function desenharBarrasDeEquipe(ctx, partida, l) {
  ...
  const vidaMaxima = Math.max(
    1,
    ...times.map((t) => t.soldados.length * 100),
  );
...
function desenharRelogio(ctx, partida, l) {
  ...
  const timeDaVez = estado.ativa ? partida.times[estado.equipeDaVez] : null;
  ctx.font = '600 10px system-ui, sans-serif';
  ctx.fillStyle = timeDaVez ? timeDaVez.cores.corpo : '#9fb0c0';
  const rotulo = timeDaVez
    ? `${timeDaVez.nome} · ${estado.ativa.nome}${timeDaVez.ia ? ' (IA)' : ''}`
    : '—';
  // A caixa do relógio é estreita (96 px): sem cortar, um nome de equipe
  // comprido (ou "(IA)" somado ao nome do soldado) sangrava para fora da
  // tela em vez de ficar dentro da caixa.
  ctx.fillText(recortar(ctx, rotulo.toUpperCase(), largura + 40), centro, y + 31);
}
...
function desenharMorteSubita(ctx, camera) {
  ...
  ctx.fillText('MORTE SÚBITA', camera.width / 2, estreito ? camera.height - RESERVA_CONTROLES - 38 - 34 : camera.height - 30);
}

/** Seta na borda quando o soldado da vez está fora da tela. */
function desenharSetaForaDaTela(ctx, estado, camera) {
  const w = estado.ativa;
  if (!w?.vivo) return;

  const p = camera.toScreen(w.x, w.y + ALTURA_SOLDADO);
  const margem = 30;
...
export function desenharDica(ctx, partida, camera) {
  ...
  const estreito = camera.width < LARGURA_ESTREITA;
  // Na tela estreita o rodapé já é do vento, da arma e da barra de controles:
  // a dica sobe para cima de tudo isso, e encurta.
  const texto = estreito
    ? '← → anda · ↑ ↓ mira · Espaço atira'
    : '← → anda · ↑ ↓ mira · Espaço segura e solta · Enter pula · [ ] arma';
  const baseY = estreito ? camera.height - RESERVA_CONTROLES - 38 - 18 : camera.height - 16;
